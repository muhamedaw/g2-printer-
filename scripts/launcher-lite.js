...
async function main() {
  process.stdout.write('\x1b]0;Money Printer G2 - Lite\x07');
  console.clear();

  p('');
  p(`${C.yellow}${C.bold}  +=========================================+${C.reset}`);
  p(`${C.yellow}${C.bold}  |   $ Money Printer G2 -- Lite Mode $    |${C.reset}`);
  p(`${C.yellow}${C.bold}  |   Low CPU  |  No Ollama  |  Heuristic  |${C.reset}`);
  p(`${C.yellow}${C.bold}  +=========================================+${C.reset}`);
  p('');

  // 1. Docker
  step('Starting Docker services (Ollama excluded)...');
  ...
  // No --profile ai  =>  ollama container is skipped
  ...
  ok('Docker services started (Ollama skipped — AI scoring uses heuristics)');
  ...
  p('');
  p(`${C.yellow}${C.bold}  +=========================================+${C.reset}`);
  p(`${C.yellow}${C.bold}  |   All systems operational! (Lite)      |${C.reset}`);
  p(`${C.yellow}${C.bold}  +=========================================+${C.reset}`);
  p('');
  p(`  ${C.gray}Dashboard :${C.reset}  ${DASHBOARD}`);
  p(`  ${C.gray}API       :${C.reset}  http://localhost:3001`);
  p(`  ${C.gray}AI mode   :${C.reset}  ${C.yellow}Heuristic (keyword scoring)${C.reset}`);
